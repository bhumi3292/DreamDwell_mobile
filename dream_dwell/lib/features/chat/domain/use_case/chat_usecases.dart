...
class CreateOrGetChatUsecase {
  ...
  Future<Map<String, dynamic>> call({required String otherUserId, String? propertyId}) =>
      repository.createOrGetChat(otherUserId: otherUserId, propertyId: propertyId);
}

class GetChatByIdUsecase {
  final ChatRepository repository;
  GetChatByIdUsecase(this.repository);
  Future<Map<String, dynamic>> call(String chatId) => repository.getChatById(chatId);
}

class GetMessagesForChatUsecase {
  final ChatRepository repository;
  GetMessagesForChatUsecase(this.repository);
  Future<List<ChatMessageModel>> call(String chatId) => repository.getMessagesForChat(chatId);
}

class SendMessageUsecase {
  final ChatRepository repository;
  SendMessageUsecase(this.repository);
  void call(String chatId, String senderId, String text) => repository.sendMessage(chatId, senderId, text);
}

class ListenForNewMessagesUsecase {
  final ChatRepository repository;
  ListenForNewMessagesUsecase(this.repository);
  void call(void Function(ChatMessageModel) callback) => repository.onNewMessage(callback);
}

class ConnectSocketUsecase {
  final ChatRepository repository;
  ConnectSocketUsecase(this.repository);
  void call(String baseUrl, String token) => repository.connectSocket(baseUrl, token);
}
