...
class SendMessageEvent extends ChatEvent {
  ...
  final String text;
  const SendMessageEvent({required this.chatId, required this.senderId, required this.text});
  @override
...
class ChatBloc extends Bloc<ChatEvent, ChatState> {
  ...
  Future<void> _onCreateOrGetChat(CreateOrGetChatEvent event, Emitter<ChatState> emit) async {
    emit(ChatLoading());
    try {
      final chat = await createOrGetChatUsecase(otherUserId: event.otherUserId, propertyId: event.propertyId);
  ...
  void _onNewMessageReceived(NewMessageReceived event, Emitter<ChatState> emit) {
    if (state is ChatLoaded) {
      ...
      final updatedMessages = List<ChatMessageModel>.from(current.messages)..add(event.message);
