...
class LandlordManageAvailability extends StatefulWidget {
  ...
  const LandlordManageAvailability({super.key, required this.propertyId});
...
class _LandlordManageAvailabilityState extends State<LandlordManageAvailability> {
  ...
  Future<void> _fetchLandlordAvailabilities() async {
    ...
    try {
      ...
      final response = await Dio().get(
        '${ApiEndpoints.baseUrl}calendar/landlord/availabilities',
        options: Options(headers: {'Authorization': 'Bearer $token'}),
      );

      final availabilities = response.data['availabilities'] as List? ?? []; // Safely cast to List and provide empty list if null
      ...
      for (var avail in availabilities) {
        ...
        if (propertyData != null && propertyData['_id'] == widget.propertyId) {
          final date = avail['date'] as String?; // date can be null from backend too
          final timeSlots = List<String>.from(avail['timeSlots'] ?? []); // timeSlots can be null or empty
          if (date != null) { // Only add if date is not null
            map[date] = timeSlots;
          }
        }
      }
      ...
      if (e is DioException) {
        if (e.response?.statusCode == 403) {
          ...
          setState(() { _error = 'Failed to fetch availability data: ${e.response?.data['message'] ?? e.message}'; });
        }
      } else {
  ...
  Future<void> _addSlot() async {
    ...
    try {
      ...
      if (e is DioException) {
        setState(() { _error = 'Failed to add time slot: ${e.response?.data['message'] ?? e.message}'; });
      } else {
  ...
  Future<void> _removeSlot(String slot) async {
    ...
    try {
      ...
      if (e is DioException) {
        setState(() { _error = 'Failed to remove time slot: ${e.response?.data['message'] ?? e.message}'; });
      } else {
  ...
  Future<void> _deleteAvailability() async {
    ...
    try {
      ...
      if (e is DioException) {
        setState(() { _error = 'Failed to delete availability: ${e.response?.data['message'] ?? e.message}'; });
      } else {
                ...
                children: [
                  ...
                  // Wrap potentially long list of slots in a SizedBox with ListView.builder
                    SizedBox(
                      ...
                      height: _slots.length * 50.0 > 200 ? 200 : _slots.length * 50.0,
                      child: ListView.builder(
                        physics: const NeverScrollableScrollPhysics(), // Prevent inner scrolling if outer scroll exists
