...
class _BookingModalState extends State<BookingModal> {
  ...
  Future<void> _bookVisit() async {
    ...
    try {
      ...
      // Using ApiEndpoints.baseUrl here
      await Dio().post('${ApiEndpoints.baseUrl}calendar/book-visit',
        data: {
      ...
      if (e is DioException) { // Use DioException for type checking
        if (e.response?.statusCode == 409) {
          ...
          setState(() { _error = 'Failed to book visit: ${e.response?.data['message'] ?? e.message}'; });
        }
      } else {
  ...
  Widget build(BuildContext context) {
    return Dialog(
      ...
      child: Padding(
        ...
        child: SingleChildScrollView( // Added SingleChildScrollView to prevent potential overflow
          child: Column(
